#ifndef HIERARCHICAL_CONTROL_SELF_COLLISION_CONTROLLER_H
#define HIERARCHICAL_CONTROL_SELF_COLLISION_CONTROLLER_H
...
//! Provides the implementation of a multiple dimensional self-collision
//avoidance task (with varying size)
class SelfCollisionsTask : public ControllerTask
{
...
public:
   ...
   * avoidance should turn on. All paris work with the same safety limit
     */
  SelfCollisionsTask(mwoibn::collision_model::RobotCollision& robot,
                     double safety_limit)
      : ControllerTask(), _robot(robot)
  {
    ...
    _init(_robot.getPairsNumber(), _robot.getRobotDofs());
  }
...
protected:
  /*		bool _init(){
                          _error =
     mwoibn::VectorN::Zero(_robot.getPairsNumber());
                          _jacobian =
     mwoibn::Matrix::Zero(_robot.getPairsNumber(),_robot.getRobotDofs());
                          _last_error =
     mwoibn::VectorN::Zero(_robot.getPairsNumber());
                          _last_jacobian =
     mwoibn::Matrix::Zero(_robot.getPairsNumber(),_robot.getRobotDofs());
