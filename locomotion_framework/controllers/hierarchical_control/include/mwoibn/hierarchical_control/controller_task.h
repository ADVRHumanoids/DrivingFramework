#ifndef HIERARCHICAL_CONTROL_CONTROLLER_TASK_H
#define HIERARCHICAL_CONTROL_CONTROLLER_TASK_H
...
*/
 class ControllerTask{
      ...
      const mwoibn::Matrix& getJacobian(){return _jacobian;}
      //! Returnes previous task Jacobian
      const mwoibn::Matrix& getPreviousJacobian(){return _last_jacobian;}

      //! Returnes task error
      const mwoibn::VectorN& getError(){return _error;}
      //! Returnes previous task error
      const mwoibn::VectorN& getPreviousError(){return _last_error;}

      //! updates task Jacobian based on the parsed argument
      void updateJacobian(mwoibn::Matrix jacobian){_last_jacobian = _jacobian; _jacobian = jacobian;}
      //! updates task error based on the parsed argument
      void updateError(mwoibn::VectorN error){_last_error = _error; _error = error;}
