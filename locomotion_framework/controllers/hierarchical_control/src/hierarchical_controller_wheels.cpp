...
bool mwoibn::hierarchical_control::HierarchicalControllerWheels::_checkStack()
{
...
//    }
    ...
    if(_state[i] != _last_state[i]){
      CastorAngleTask& cast = dynamic_cast<CastorAngleTask&>(_task_low);
      if(!_state[i])
...
void mwoibn::hierarchical_control::HierarchicalControllerWheels::_findTasks()
{
  auto it = std::find(_tasks_ptr.begin(), _tasks_ptr.end(), &_task_high);

  if (it == _tasks_ptr.end())
    throw(std::invalid_argument("HierarchicalControllerWheels: couldn't find a "
                                "task_high in a tasks stack."));
  _id_high = it - _tasks_ptr.begin();

  it = std::find(_tasks_ptr.begin(), _tasks_ptr.end(), &_task_low);

  if (it == _tasks_ptr.end())
    throw(std::invalid_argument("HierarchicalControllerWheels: couldn't find a "
                                "task_low in a tasks stack."));
  _id_low = it - _tasks_ptr.begin();
}

//void mwoibn::hierarchical_control::HierarchicalControllerWheels::_updateTask(
//    int i, mwoibn::hierarchical_control::ControllerTask* task)
//{
...
void mwoibn::hierarchical_control::HierarchicalControllerWheels::compute()
{
  _command.setZero();
  _P.setIdentity();
...
//  i = 0;
  ...
  for (int k = 0; k < _tasks_ptr.size()-2; k++)
  {
      auto& task = _tasks_ptr[k];

      _errors[k].noalias() = -(_gains[k].asDiagonal() * task->getError());
      _errors[k].noalias() -= task->getJacobian() * _command;
