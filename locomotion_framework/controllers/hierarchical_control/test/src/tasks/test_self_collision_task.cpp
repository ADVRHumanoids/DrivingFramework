...
*/
  ...
  std::string file_name = path + "/test/resources/self_collision_controller.txt";
  ...
  Eigen::MatrixXd J_1 = mwoibn::tests_common::readMatrix(&myfile, 1, 15);

  if (!mwoibn::tests_common::skipLine(&myfile, 1))
    FAIL() << "couldn't continue, file finished prematurely";
  Eigen::VectorXd e_1 = mwoibn::tests_common::readMatrix(&myfile, 1, 1);

  if (!mwoibn::tests_common::skipLine(&myfile, 1))
    FAIL() << "couldn't continue, file finished prematurely";
  Eigen::MatrixXd J_2 = mwoibn::tests_common::readMatrix(&myfile, 1, 15);

  if (!mwoibn::tests_common::skipLine(&myfile, 1))
    FAIL() << "couldn't continue, file finished prematurely";
  Eigen::VectorXd e_2 = mwoibn::tests_common::readMatrix(&myfile, 1, 1);
#endif
