...
TEST(ControlerTaskTest, methodsBaseClass)
{
  ...
  mwoibn::VectorN reference(6);

  reference <<  0.3, -0.5, 0, 0.3, 0.5, 0;
  ...
  std::string file_name = path + "/test/resources/cartesian_world_task.txt";
  ...
  Eigen::MatrixXd J_1 = mwoibn::tests_common::readMatrix(&myfile, 6, 15);

  if (!mwoibn::tests_common::skipLine(&myfile, 1))
    FAIL() << "couldn't continue, file finished prematurely";
  Eigen::VectorXd e_1 = mwoibn::tests_common::readMatrix(&myfile, 6, 1);

  if (!mwoibn::tests_common::skipLine(&myfile, 1))
    FAIL() << "couldn't continue, file finished prematurely";
  Eigen::MatrixXd J_2 = mwoibn::tests_common::readMatrix(&myfile, 6, 15);
