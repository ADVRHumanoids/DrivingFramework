...
//            config["source"].as<std::string>(), 1,
//            boost::bind(&RosOperationalEuler::_init, this, _1, &started));
        ros::Rate rate(10);
        ...
        while (ros::ok() && !_initialized && tries < max_tries)
        {
          ...
          std::cout << "Waiting for feedback from " +
                           ...
                           " to initialized. Try " << std::to_string(tries+1) << "/" << max_tries << std::endl;
          tries++;
          rate.sleep();
        }

        if(_initialized)
        std::cout << "Feedback from " +
                         config["source"].as<std::string>() +
      ...
      std::cout << "Loaded ROS operational feedback " << config["name"] << std::endl;
