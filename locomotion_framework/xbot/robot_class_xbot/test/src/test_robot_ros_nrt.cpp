...
TEST(RobotClassTest, methodsRosNRT)
{
  try
  {
    ...
    mwoibn::VectorN orignal_states = robot.state.get(mwoibn::robot_class::INTERFACE::POSITION);
...
;
    ...
    EXPECT_FALSE(robot.state.get(mwoibn::robot_class::INTERFACE::POSITION).norm() == orignal_states.norm());

    // check if both controller and mapping are working properly
    EXPECT_NEAR(joint_states.tail(robot.getDofs()-6).norm(), robot.state.get(mwoibn::robot_class::INTERFACE::POSITION).tail(robot.getDofs()-6).norm(), 0.1);
    EXPECT_NEAR(joint_states[6], robot.state.get(mwoibn::robot_class::INTERFACE::POSITION)[6], 0.1);
    EXPECT_NEAR(joint_states[7], robot.state.get(mwoibn::robot_class::INTERFACE::POSITION)[7], 0.01);
    EXPECT_NEAR(joint_states[robot.getDofs() - 1],
                robot.state.get(mwoibn::robot_class::INTERFACE::POSITION)[robot.getDofs() - 1], 0.01);
  }
  catch (...)
  {
    ADD_FAILURE() << "Uncaught exception";
...
;
    ...
    EXPECT_TRUE(mwoibn::tests_common::compareMatrices(robot.state.get(mwoibn::robot_class::INTERFACE::POSITION).tail(14), orignal_states.tail(14), eps));
    ...
    robot.command.set(zero, mwoibn::robot_class::INTERFACE::TORQUE);
...
;
    ...
    robot.command.set(zero, mwoibn::robot_class::INTERFACE::POSITION);
    EXPECT_TRUE(robot.controllers.send());
...
;
    ...
    EXPECT_FALSE(mwoibn::tests_common::compareMatrices(robot.state.get(mwoibn::robot_class::INTERFACE::POSITION).tail(14), orignal_states.tail(14), 10*eps));
...
;
    ...
    robot.command.set(feed_forward_controller, mwoibn::robot_class::INTERFACE::TORQUE);
...
;
    ...
    EXPECT_FALSE(mwoibn::tests_common::compareMatrices(robot.state.get(mwoibn::robot_class::INTERFACE::POSITION).tail(25), orignal_states.tail(25), 10*eps));
    std::cout << "robot.state.get(mwoibn::robot_class::INTERFACE::POSITION)" << std::endl;
    std::cout << robot.state.get(mwoibn::robot_class::INTERFACE::POSITION) << std::endl;
