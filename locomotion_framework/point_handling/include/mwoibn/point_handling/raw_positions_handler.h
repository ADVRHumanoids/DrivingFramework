...
public:
  ...
  RawPositionsHandler(RawPositionsHandler&& other)
  ...
  getPointJacobian(unsigned int id,
                   const mwoibn::VectorN& joint_states, bool update = false) const
  {
  ...
  virtual unsigned int addPoint(State state, int body_id,
                        std::string name = ""){
      _points.push_back(
          std::unique_ptr<Point>(new Point(state, body_id, _model, Point::Orientation(0,0,0,1), name)));
  ...
  void setPointStateFixed(unsigned int id,
                          const State state)
  {
  ...
  virtual void
  setPointStateWorld(unsigned int id, const State state,
                     const mwoibn::VectorN& joint_states, bool update = false)
  {
    _points.at(id)->setPositionWorld(state, joint_states, update);
  }

  virtual const State&
  getPointStateWorld(unsigned int id,
                     const mwoibn::VectorN& joint_states, bool update = false)
  {
  ...
  getPointStateReference(unsigned int id,
                         const mwoibn::VectorN& joint_states, bool update = false)
  {
    return _points.at(id)->getPositionReference(_reference, joint_states, update);
  }

  virtual void
  setPointStateReference(unsigned int id, const State state,
                         const mwoibn::VectorN& joint_states, bool update = false)
  {
    _points.at(id)->setPositionReference(state, _reference, joint_states, update);
  }
                                     ...
                                     const mwoibn::VectorN& joint_states, bool update = false)
  {
    ...
    for (int i = 0; i < size(); i++)
    {
      _state.noalias() = state.segment(
          k, _points.at(i)->size()); // here the proper size has to ensured by a user
