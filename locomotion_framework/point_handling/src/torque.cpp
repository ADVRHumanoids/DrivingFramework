...
namespace point_handling
{
  ...
  void Torque::getWorld(Point::Current& angular, bool update) const {
        ...
        angular.head<3>() = frame.rotation().getWorld(update)*(_current + frame.position.getWorld(false).head<3>().cross(force.getFixed().head<3>()));
        return;
  }
  ...
  void Torque::getReference(Point::Current& current, unsigned int reference_id, bool update) const {
        current.head<3>()= frame.rotation().getReference(reference_id, update)*(_current + frame.position.getReference(reference_id, false).head<3>().cross(force.getFixed().head<3>()));
  }
