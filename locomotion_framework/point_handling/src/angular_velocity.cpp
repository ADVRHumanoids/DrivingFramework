...
namespace point_handling
{
  ...
  const mwoibn::Matrix& AngularVelocity::getJacobian(bool update)  {
      ...
      _J_full.setZero();
      CalcPointJacobian6D(_model, _state.get(), _body_id, frame.position.getFixed(), _J_full, update);
  ...
  mwoibn::Matrix AngularVelocity::getJacobian(bool update) const{
    ...
    CalcPointJacobian6D(_model, _state.get(), _body_id, frame.position.getFixed(), J_full, update);
