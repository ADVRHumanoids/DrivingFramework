...
TEST(FullStatePHTest, initialization)
{
  ...
  mwoibn::VectorN P_1 =
      mwoibn::VectorN::Zero(7);
  P_1 << 0.0, 0.0, -0.15, 0, 0, 0, 1;
  ...
  EXPECT_THROW(
      mwoibn::point_handling::RawFullStatesHandler point_2(
  ...
  EXPECT_THROW(
      ...
      std::invalid_argument);
  EXPECT_THROW(mwoibn::point_handling::RawFullStatesHandler point_2(
  ...
  mwoibn::point_handling::RawFullStatesHandler point_1("pelvis",
                                                    *(_model_ptr));
  mwoibn::point_handling::RawFullStatesHandler point_2(
      _model_ptr->GetBodyId("pelvis"), *(_model_ptr));
...
TEST(FullStatePHTest, JacobianAndStateMethods)
{


  RigidBodyDynamics::Model* _model_ptr = new RigidBodyDynamics::Model;
  // Try to load a RBDL model, if fail abort, as point_handling is dependend on it
  ...
  {
    ADD_FAILURE() << "Cannot load rbdl model abort:" << exc.what();
  }
  catch (...)
  {
    FAIL() << "Cannot load rbdl model abort";
