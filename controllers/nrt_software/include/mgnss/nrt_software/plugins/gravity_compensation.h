...
class GravityCompensation : public mgnss::plugins::RosBase
{

public:
GravityCompensation() : mgnss::plugins::RosBase(){}

mgnss::controllers::CentralizedController& get(){
        return static_cast<mgnss::controllers::CentralizedController&>(*_controller_ptr);
}
...
virtual void _resetPrt(YAML::Node config){
        _controller_ptr.reset(new mgnss::controllers::CentralizedController(*_robot_ptr.begin()->second, config));
}
