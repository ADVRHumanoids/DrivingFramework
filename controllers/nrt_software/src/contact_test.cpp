...
public:
  ...
  mwoibn::Vector3& getContact(int i) { return _current[i]; }

  mwoibn::Vector3& getEstimate(int i) { return _estimated[i]; }
  mwoibn::Vector3& getSpherical(int i) { return _spherical[i]; }

  mwoibn::Vector3& getModelDiff(int i) { return _modelDiff[i]; }
  mwoibn::Vector3& getEquationDiff(int i) { return _equationDiff[i]; }
...
//    temp += _skew(_ground) * r;
  ...
  void _flowEstimate(int i) { _estimated[i] += _flowDiff[i] * _robot.rate(); }
