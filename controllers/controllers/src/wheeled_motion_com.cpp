...
void mwoibn::WheeledMotionCom::nextStep(const mwoibn::VectorN& support)
{
...
void mwoibn::WheeledMotionCom::resetSteering()
{
  for (int i = 0; i < 4; i++)
  {
...
double mwoibn::WheeledMotionCom::limit(const double th)
{
  return th - 6.28318531 * std::floor((th + 3.14159265) / 6.28318531);
}

void mwoibn::WheeledMotionCom::update(const mwoibn::VectorN& support)
{
...
void mwoibn::WheeledMotionCom::fullUpdate(const mwoibn::VectorN& support)
{
  _robot.get();
  _robot.updateKinematics();
...
void mwoibn::WheeledMotionCom::compute()
{
//_leg_castor_ptr->updateError();
//  _leg_steer_ptr->updateError();
