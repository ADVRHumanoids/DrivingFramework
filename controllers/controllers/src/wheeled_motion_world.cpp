...
void mgnss::controllers::WheeledMotionWorld::_createTasks(YAML::Node config){
        ...
        _steering_ptr.reset(
                new mwoibn::hierarchical_control::tasks::CartesianFlatReferenceWorld(
...
/*
   ...
   logger.addField("base_x", _robot.state.position.get()[0]);
   logger.addField("base_y", _robot.state.position.get()[1]);
   logger.addField("base_z", _robot.state.position.get()[2]);
