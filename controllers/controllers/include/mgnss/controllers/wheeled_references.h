...
const mwoibn::VectorN& getDesired(){
        return _desired;
}
const mwoibn::VectorN& getBase(){
        return _base;
}
...
bool isDone() {
        return (std::fabs(_t - _limit) < _step);
}
...
virtual void setDesired(double desired) {
        _desired[0] = desired;
}
