...
#include <mwoibn/point_handling/spatial_acceleration.h>
...
void mgnss::controllers::WheelsZMPII::compute()
{
        ...
        mwoibn::Matrix Pn(2,3);
        Pn << 1,0,0,  0,1,0;
        mwoibn::Vector3 n;
        n << 0,0,1;
        double gain = 0.01;
        //double F_min = 100, F_max = 140, saturation = 1;

        // Newton Equation
        mwoibn::VectorN state(1);
        _force_opt.equality[0].setJacobian().setOnes();
        mwoibn::Vector3 gravity = _robot.centerOfMass().mass()*_robot.gravity();
        _force_opt.equality[0].setState() = gain*n.transpose()*gravity;
        for (auto& contact :  _robot.contacts().group("wheels"))
          _force_opt.equality[0].setState() += gain*n.transpose()*contact->wrench().force.getWorld();


        // // Eulqer Equation
        //_force_opt.equality[1].setState();
        mwoibn::Matrix3 skew = mwoibn::Matrix3::Zero();
        mwoibn::Vector3 point;
        _force_opt.equality[1].setState().setZero();
        for (auto&& [id, contact] :  ranges::view::enumerate(_robot.contacts().group("wheels"))){
          point = contact->get() - _robot.centerOfMass().get();
          mwoibn::eigen_utils::skew(point, skew);
          _force_opt.equality[1].setJacobian().col(id) = Pn*skew*n;
          _force_opt.equality[1].setState() += Pn*skew*gain*n*n.transpose()*contact->wrench().force.getWorld();
          std::cout << contact->getGroundNormal().transpose()*contact->wrench().force.getWorld() << std::endl;
        }

        // Inequality min/max constraint
        for(auto&& [id, contact] :  ranges::view::enumerate(_opt_forces)){
          _momentum_jacobian(id,contact) = 1;
          _momentum_min[id] = gain*(F_min[contact] - n.transpose()*_robot.contacts()[contact].wrench().force.getWorld());
          _momentum_max[id] = gain*(F_max[contact] - n.transpose()*_robot.contacts()[contact].wrench().force.getWorld());
        }

        _force_opt.solve();
        // std::cout << "equality state" << std::endl;
        // std::cout << _force_opt.equality.getState().transpose() << std::endl;
        // std::cout << "equality jacobian" << std::endl;
        // std::cout << _force_opt.equality.getJacobian() << std::endl;
        // std::cout << "inequality state" << std::endl;
        // std::cout << _force_opt.hard_inequality.getState().transpose() << std::endl;
        // std::cout << "inequality jacobian" << std::endl;
        // std::cout << _force_opt.hard_inequality.getJacobian() << std::endl;
        std::cout << "solution" << std::endl;
        std::cout << _force_opt.get().transpose() << std::endl;
        std::cout << "limits" << std::endl;
        std::cout << F_min.transpose() << std::endl;
        std::cout << F_max.transpose() << std::endl;

        // NRT TEMP HEARCODED
        // double F_min = 100, F_max = 140, saturation = 1;
        // double gain = 0.01;
        mwoibn::VectorN velocities(12);
        // mwoibn::Matrix _jacobian(4, _robot.getDofs());
        // _jacobian.setZero();
        // _base.update(true);
        for (auto&& [id, contact] :  ranges::view::enumerate(_robot.contacts().group("wheels"))){
           velocities.segment<3>(3*id) = contact->getGroundNormal()*_force_opt.get()[id];
        }


        _tasks["CONSTRAINTS"]->setVelocity(velocities);
        // std::cout << "x_base\t" << x_base.transpose() << std::endl;
        // std::cout << "x_des\t" << x_des.transpose() << std::endl;
        // std::cout << "base.get\t" << _base[1].get().transpose() << std::endl;

        //std::cout << "current\t" << _base[1].get().transpose() << std::endl;
        //std::cout << "jacobian\n" << _base[1].getJacobian() << std::endl;
        //std::cout << "point\n" << _base[0].getJacobian() << std::endl;
        //std::cout << "point\n" << _base[2].getJacobian() << std::endl;
        ...
        // _contacts_inverse->compute(_jacobian);
        //std::cout << "torque\t" << _robot.state.torque.get().transpose() << std::endl;
        //std::cout << "overall_force\t" << _robot.state["OVERALL_FORCE"].get().transpose() << std::endl;
        // std::cout << _contacts_inverse->get().rows() << "\t" << _contacts_inverse->get().cols() << std::endl;
        // std::cout << "_contacts_inverse" << std::endl;
        // std::cout << _contacts_inverse->get()*velocities << std::endl;
        // std::cout << "_tasks[BASE]" << std::endl;
        // std::cout << _tasks["BASE"]->getJacobian() << std::endl;
        // std::cout << "velocities" << std::endl;
        // std::cout << velocities.transpose() << std::endl;
        // _tasks["BASE"]->update();
        // mwoibn::VectorN base = -_tasks["BASE"]->getJacobian().leftCols<30>()*_contacts_inverse->get()*velocities;
        // std::cout <<"estimate " << base.transpose() << std::endl;
        // std::cout <<"current " << (_tasks["BASE"]->getJacobian()*_robot.state.velocity.get()).transpose() << std::endl;
        // _tasks["BASE"]->setVelocity(base);
        //_support = _support + _support_vel*_robot.rate();
        // without resttering
        WheelsController::compute();
        //std::cout << "getVelocity\t" << _tasks["BASE"]->getVelocity().transpose() << std::endl;
...
void mgnss::controllers::WheelsZMPII::_allocate(){
      ...
      _contacts_inverse.reset(new mwoibn::PseudoInverse(mwoibn::Matrix(12, 30 ), 1e-8));
...
//        _robot.states[GAINS].velocity.set(temp_);
          // velocities.setZero(12);
          _force_opt.resize(4, 0);
          _momentum_jacobian.setZero(_opt_forces.size(), 4);
          _momentum_min.setZero(_opt_forces.size());
          _momentum_max.setZero(_opt_forces.size());
          F_max.setConstant(4, 300);
          F_min.setConstant(4, 50);
}
...
void mgnss::controllers::WheelsZMPII::_initIK(YAML::Node config){
    ...
    _force_opt.equality.add(mgnss::higher_level::Constraint(1,4)); // Newton Equation
    _force_opt.equality.add(mgnss::higher_level::Constraint(2,4)); // Euler Equation

    //mgnss::higher_level::constraints::Merge _contact_limits;
    //_contact_limits.add(mgnss::higher_level::constraints::MaximumLimit(_momentum_jacobian, _momentum_max));
    //_contact_limits.add(mgnss::higher_level::constraints::MinimumLimit(_momentum_jacobian, _momentum_min));
    //_contact_limits.active_dofs = mwoibn::eigen_utils::iota(4);
    //_contact_limits.active.setConstant(_opt_forces.size(), true);

    //_force_opt.addSoft(_contact_limits, 1e3);
    //_force_opt.addSoft(_contact_limits, 1e3);
    //_force_opt.addSoft(_contact_limits, 1e3);
    _force_opt.addSoft(mgnss::higher_level::constraints::MaximumLimit(_momentum_jacobian, _momentum_max), 1e3);
    _force_opt.addSoft(mgnss::higher_level::constraints::MinimumLimit(_momentum_jacobian, _momentum_min),1e3);
    _force_opt.soft_inequality[0].init();
    _force_opt.soft_inequality[1].init();

    _force_opt.init();
    //_force_opt.verbose = true;
    _robot.state.add("DESIRED_FORCES",_robot.getDofs());
//    _base.add(mwoibn::dynamic_points::Acceleration<mwoibn::point_handling::SpatialAcceleration, mwoibn::point_handling::SpatialVelocity>("pelvis", _robot));
//    _base.add(mwoibn::dynamic_points::Force(_robot, *_dynamic_ptr, _base[0], "OVERALL_FORCE"));
//    _base.add(mwoibn::dynamic_points::ForceBase(_robot, *_dynamic_ptr, _base[0], "OVERALL_FORCE"));
    // std::shared_ptr estimator_ = std::make_shared<mwoibn::hierarchical_control::actions::Estimator>(mwoibn::hierarchical_control::actions::Estimator(_ik_ptr->state, *_actions["CONSTRAINTS"],  _ik_ptr->map()));
    // _actions["CONSTRATINS_ESTIMATOR"] = estimator_;
    // _ik_ptr->addAfter(*_actions["CONSTRATINS_ESTIMATOR"], *_actions["CONSTRAINTS"]);
    //
    // _actions["CONSTRATINS_RESTORE"] = std::make_shared<mwoibn::hierarchical_control::actions::Restore<mwoibn::hierarchical_control::actions::Estimator>>(mwoibn::hierarchical_control::actions::Restore(*estimator_, _ik_ptr->state, _ik_ptr->map()));
    // _ik_ptr->addAction(*_actions["CONSTRATINS_RESTORE"]);
