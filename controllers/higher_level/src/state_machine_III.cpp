#include "mgnss/higher_level/state_machine_III.h"

mgnss::higher_level::StateMachineIII::StateMachineIII(mwoibn::robot_class::Robot& robot, YAML::Node config):
  ...
  //   _contact_points.add(mwoibn::robot_class::name, )
        ...
        for(auto& contact: _robot.contacts())
        {
            ...
            cost_I.torus_acceleration.add(mwoibn::dynamic_points::Torus(_contact_points.end(0), _robot ) );
        ...
        _margins.limit << 0.25, 0.215, 0.215, 0.25;
        ...
        cost_I.offset.set().setZero(_size*3);
        cost_I.jacobian.set().setZero(_size*3, 3*_size);

        cost_II.offset.set().setZero(_size*3);
        cost_II.jacobian.set().setZero(_size*3, _robot.getDofs());


        _margins.setJacobian().setZero(_size, vars); // cp + base
        _workspace.setJacobian().setZero(_size, vars);

        state_I.offset.set().setZero(vars);
        state_I.jacobian.set().setZero(vars, vars);
...
void mgnss::higher_level::StateMachineIII::init(){ }



// I should check if it works before making the full optimization
void mgnss::higher_level::StateMachineIII::_computeMargin(int i){
...
void mgnss::higher_level::StateMachineIII::_marginJacobians(){
...
void mgnss::higher_level::StateMachineIII::update(){
  _update();
  ...
  state_I.jacobian.set().block(0,0,3*_size, 3*_size) = cost_I.jacobian.get();
  state_I.offset.set().head(3*_size) = cost_I.offset.get();
...
void mgnss::higher_level::StateMachineIII::_computeWorkspace(){
  for(int i = 0; i < _contact_points.size(); i++)
    _workspace.setState()[i] = _workspace_points[i].get().transpose()*_workspace_points[i].get();
}

void mgnss::higher_level::StateMachineIII::_workspaceJacobian(){
  for(int i = 0; i < _contact_points.size(); i++){
...
void mgnss::higher_level::StateMachineIII::_update(){
...
void mgnss::higher_level::StateMachineIII::log(mwoibn::common::Logger& logger){
...
void mgnss::higher_level::SupportStateII::update(){
  ...
  for(int i = 0; i < torus_acceleration.size(); i++){
    ...
    jacobian.set().block<3,3>(3*i, 3*i) = _support_jacobian;
    offset.set().segment<3>(3*i) = _support_offset;
