...
mgnss::higher_level::QRJointSpaceV3::QRJointSpaceV3(mgnss::higher_level::QrTask& task, const mwoibn::Matrix& jacobian,
   const mwoibn::VectorN& offset, mwoibn::robot_class::Robot& robot, const mwoibn::VectorInt& chain, double damping):
    mgnss::higher_level::QrTask(), _robot(robot), _task(task), _jacobian(jacobian), _offset(offset), _damping(damping), _chain(chain){

}

void mgnss::higher_level::QRJointSpaceV3::init(){
  _task.init();
  resize(_chain.size(), _task.soft_inequality.rows());

  std::cout << "_chain\t" << _chain.transpose() << std::endl;
  _damp_vec.setConstant(_vars, _damping);
  _slack = _task.soft_inequality.rows();
  _active_jacobian.setZero(_jacobian.rows(), _chain.size());
//  equality.clear();
...
//  hard_inequality.clear();
  ...
  _temp.setZero(_task.vars(), _chain.size());
  // _cost.quadratic.block(0,0,_vars, _vars).setIdentity();

}

void mgnss::higher_level::QRJointSpaceV3::_update(){
      _task._update();
     // _marginJacobians();
     _optimal_state.setZero();// = _robot.command.velocity.get(); // init from last command?

     for(int i = 0; i < _chain.size(); i++)
       _active_jacobian.col(i) = _jacobian.col(_chain[i]);

     for(auto&& zip: ranges::view::zip(_task.equality,
                                          ranges::view::slice(equality, ranges::end - _task.equality.size(),
                                          ranges::end) ) )
   {
       int old_ = std::get<0>(zip)->getJacobian().cols();
       int new_ = _chain.size();
       ...
       std::get<1>(zip)->setJacobian().block(0,0,size_, new_).noalias() = std::get<0>(zip)->getJacobian().leftCols(old_)*_active_jacobian;
    }

    for (auto&& zip: ranges::view::zip(_task.soft_inequality,
                     ranges::view::slice(soft_inequality, ranges::end - _task.soft_inequality.size(),
                     ranges::end) ) )
    {
     int old_ = std::get<0>(zip)->getJacobian().cols();
     int new_ = _chain.size();
    ...
    for (auto&& zip: ranges::view::zip(_task.hard_inequality,
                     ranges::view::slice(hard_inequality, ranges::end - _task.hard_inequality.size(),
                        ranges::end) ) )
    {
     int old_ = std::get<0>(zip)->getJacobian().cols();
     int new_ = _chain.size();
  ...
  _temp.noalias() = _task.cost().quadratic.block(0,0,_task.vars(), _task.vars()) * _active_jacobian;
  _cost.quadratic.block(0,0,_vars, _vars).noalias()  = _active_jacobian.transpose() * _temp;
  ...
  _cost.linear.head(_vars).noalias()  = _task.cost().linear.head(_task.vars()).transpose() * _active_jacobian;
...
void mgnss::higher_level::QRJointSpaceV3::log(mwoibn::common::Logger& logger){
...
void mgnss::higher_level::QRJointSpaceV3::_outputTransform(){
  ...
  _return_state.noalias() = _active_jacobian*_optimal_state.head(_vars);
