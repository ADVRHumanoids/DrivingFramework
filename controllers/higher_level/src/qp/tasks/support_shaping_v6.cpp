#include "mgnss/higher_level/qp/tasks/support_shaping_v6.h"



mgnss::higher_level::SupportShaping6::SupportShaping6(mwoibn::robot_class::Robot& robot, YAML::Node config,  std::vector<std::unique_ptr<mwoibn::robot_points::Rotation>>& steering_frames, const mgnss::higher_level::Limit& margin, const mgnss::higher_level::Limit& workspace, bool is_margin):
  QrTask(12+2+12, 8), _robot(robot), _margin(margin), _wheel_transforms(steering_frames), _workspace(workspace), _is_margin(is_margin){
    if(!_is_margin) resize(8,0);
}

void mgnss::higher_level::SupportShaping6::_allocate(){
          ...
          hard_inequality.clear();
            ...
            if(_is_margin)
              ...
              for(int i = 0; i < 4; i++){
                ...
                _cost.quadratic(3*i, 3*i) = 1;
                _cost.quadratic(3*i+1, 3*i+1) = 1;
...
void mgnss::higher_level::SupportShaping6::_update(){

     _optimal_state.setZero();

    for(int i = 0; i < _size; i++){
      if(_is_margin)
        soft_inequality[0].setJacobian().block<4,2>(0,3*i) = _margin.getJacobian().middleCols<2>(3*i);
    ...
    //    soft_inequality[1].setState()[4+i] = -(0.0*0.0 - _workspace.getState()[i])/_robot.rate(); //Avoid going under the robot?
        soft_inequality[1].setJacobian().block<4,2>(0,3*i) = -_workspace.getJacobian().middleCols<2>(3*i);
        soft_inequality[1].setJacobian().block<4,2>(0,3*_size+2+3*i) = -_workspace.getJacobian().middleCols(3*_size+2+3*i,2);
    ...
    soft_inequality[0].setJacobian().block<4,2>(0,3*_size) = _margin.getJacobian().middleCols<2>(3*_size);
...
void mgnss::higher_level::SupportShaping6::log(mwoibn::common::Logger& logger){
...
void mgnss::higher_level::SupportShaping6::_outputTransform(){
  ...
  for(int i = 0; i < 4; i++){
    ...
    _return_state.segment<2>(2*i) = _optimal_state.segment<2>(3*i);
