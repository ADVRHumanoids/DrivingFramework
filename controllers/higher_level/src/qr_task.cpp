...
mgnss::higher_level::QrTask::QrTask(int vars, int slack): soft_inequality(vars+slack),
          hard_inequality(vars+slack), equality(vars+slack), _vars(vars), _slack(slack), _llt(vars){

          _cost.size = vars + slack;

          _optimal_state.setZero(_cost.size); // state + slack

          _cost.quadratic.setZero(_cost.size,_cost.size); // this should work only with lower body - how?
...
void mgnss::higher_level::QrTask::solve(){
    ...
    update();
    //
    // std::cout << "_cost.quadratic\t" << _cost.quadratic << std::endl;
