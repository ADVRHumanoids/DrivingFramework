...
public:
  ...
  virtual void updateError()
  {
    ...
    for (int i = 0; i < _ik.size(); i++)
    {
      ...
      _error.segment(3 * i, 2) =
          _reference.segment(2 * i, 2) - _point_flat.head(2);
       _error[3 * i + 2] = _hight[i] - _point_flat[2];
    }
  }
  ...
  void updateState()
  {
    ...
    if (std::fabs(temp[0] - _state[2]) > 1.57079633 &&
        std::fabs(temp[0] - _state[2]) < 4.71238898)
    {
    ...
    // Try controlling a contact point with respect to the body/ not a wheel center
    for(int i = 0; i < _ik.size(); i++){
      mwoibn::Vector3 position = _robot.contacts().contact(i).getPosition().head(3);
      _ik.setPointStateWorld(i, position);
    }
//    std::cout << "_ik.getFullStateWorld()" << std::endl;
//    std::cout << _ik.getFullStateWorld() << std::endl;
  }
...
//        _ik.getFullJacobianRows(), _ik.getFullJacobianCols());
    ...
    mwoibn::Matrix new_state = mwoibn::Matrix::Zero(_ik.size() * 3, 6);
    ...
    for (int i = 0; i < _ik.size(); i++)
    {
      ...
      new_state.block(3 * i, 0, 3, 6) = new_point_state.topRows(3);
    }
    ...
    mwoibn::Matrix jacobian =
        (mwoibn::eigen_utils::pseudoInverse<mwoibn::Matrix>(
         ...
         new_jacobian.leftCols(6));

//    std::cout << "new_jacobian\t" << new_jacobian.leftCols(6) << std::endl;
//    std::cout << "new_state\t" << new_state.leftCols(6) << std::endl;
//    std::cout << "jacobian\t" << jacobian << std::endl;
//    std::cout << "pseudo\t" << mwoibn::eigen_utils::pseudoInverse<mwoibn::Matrix>(
//                   new_state.leftCols(6), 1e-8) << std::endl;
    ...
    _jacobian.rightCols(1) = mwoibn::Matrix::Zero(_jacobian.rows(), 1);

    for (int i = 0; i < _ik.size(); i++)
      _jacobian.block(3 * i, 0, 2, _robot.getDofs()) =
          rotation * _jacobian.block(3 * i, 0, 2, _robot.getDofs());
  ...
  bool setHight(mwoibn::VectorN hights)
  {
    if (_hight.size() == hights.size())
    {
      _hight = hights;
      return true;
    }
