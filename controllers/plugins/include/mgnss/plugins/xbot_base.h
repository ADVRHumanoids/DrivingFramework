...
public:
  XbotBase(int argc, char** argv, std::string name): _name(name){
  }

  XbotBase(std::string name): _name(name){
  }
...
protected:
virtual void control_loop(double time, double period){control_loop(time);}
...
std::string _name = "";
double _start;
XBot::RosUtils::RosHandle::Ptr _n;
};
}
}
#endif // RT_MY_TEST_H
