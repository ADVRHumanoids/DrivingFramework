...
public:
	...
	QuadProg(int vars, int equality, int inequality){
		init(vars, equality, inequality);
	}

	void init(int vars, int equality, int inequality){
		n=vars;
		p=equality;
		m=inequality;
		R.setZero(n,n);
...
protected:
  ...
  int iq, iter = 0;
	...
	VectorXd s, z, r, d,  np, u;
	VectorXd x_old, u_old;
	...
	VectorXi A, A_old, iai, iaexcl;


	bool _add_constraint(MatrixXd& R, MatrixXd& J, VectorXd& d, int& iq, double& R_norm);
  void _delete_constraint(MatrixXd& R, MatrixXd& J, VectorXi& A, VectorXd& u,  int p, int& iq, int l);


	inline void _compute_d(VectorXd &d, const MatrixXd& J, const VectorXd& np)
	{
		d.noalias() = J.adjoint() * np;
	}

	inline void _update_z(VectorXd& z, const MatrixXd& J, const VectorXd& d,  int iq)
	{
		z.noalias() = J.rightCols(z.size()-iq) * d.tail(d.size()-iq);
	}

	inline void _update_r(const MatrixXd& R, VectorXd& r, const VectorXd& d, int iq)
	{
...
inline double QuadProg::solve_quadprog(MatrixXd & G,  VectorXd & g0,
                      const MatrixXd & CE, const VectorXd & ce0,
                      const MatrixXd & CI, const VectorXd & ci0,
                      VectorXd& x){
...
inline double QuadProg::solve_quadprog2(LLT<MatrixXd,Lower> &chol,  double c1, VectorXd & g0,
                      const MatrixXd & CE, const VectorXd & ce0,
                      const MatrixXd & CI, const VectorXd & ci0,
                      VectorXd& x)
{
...
// #endif
    ...
    if (!_add_constraint(R, J, d, iq, R_norm))
    {
      // FIXME: it should raise an error
      // Equality constraints are linearly dependent
			std::cout << "Equality constraints are linearly dependent" << std::endl;
      return f_value;
    }
  }
...
// #endif
  ...
  if (t2 >= inf)
  {
    ...
    u(iq) += t;
    iai(l) = l;
    _delete_constraint(R, J, A, u, p, iq, l);
#ifdef TRACE_SOLVER
  ...
  if (t == t2)
  {
    ...
    /* add constraint ip to the active set*/
		...
		{
			iaexcl(ip) = 0;
			_delete_constraint(R, J, A, u, p, iq, ip);
// #ifdef TRACE_SOLVER
      // print_matrix("R", R, n);
...
inline bool QuadProg::_add_constraint(MatrixXd& R, MatrixXd& J, VectorXd& d, int& iq, double& R_norm)
{
 int n=J.rows();
#ifdef TRACE_SOLVER
...
inline void QuadProg::_delete_constraint(MatrixXd& R, MatrixXd& J, VectorXi& A, VectorXd& u,  int p, int& iq, int l)
{
